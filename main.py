...
def cleanup_libreoffice():
    """Clean up LibreOffice lock files and profiles to prevent corruption."""
    try:
        # Remove lock files
        for lock_file in Path('/tmp').glob('.~lock.*'):
            try:
                lock_file.unlink()
        ...
        # Remove any stale LibreOffice temp directories
        for tmp_dir in Path('/tmp').glob('lu*'):
            if tmp_dir.is_dir():
                try:
        ...
        # Clear user profile if corrupted
        lo_profile = Path.home() / '.config' / 'libreoffice'
        if lo_profile.exists():
            try:
                shutil.rmtree(lo_profile)
                logger.info("Cleared LibreOffice user profile")
            except:
                pass
...
def analyze_xlsx_worksheets(xlsx_path: str) -> dict:
        ...
        with zipfile.ZipFile(xlsx_path, 'r') as zf:
            ...
            for f in file_list:
                if 'drawings/_rels/drawing' in f and f.endswith('.rels'):
                    try:
                        ...
                        if match:
                            drawing_image_count[match.group(1)] = image_count
                    except:
                        pass
            ...
            for ws in worksheets:
                ...
                try:
                    ...
                    # Image scoring - MORE images = higher score
                    if sheet_num in worksheet_to_drawing:
                        ...
                        logger.info(f"Sheet {sheet_num}: {img_count} images -> score contribution")
    ...
    if worksheet_scores:
        ...
        logger.info(f"Selected sheet: {best_sheet} with score {worksheet_scores[best_sheet]}")
    else:
        best_sheet = 1
    
    return {'scores': worksheet_scores, 'best_sheet': best_sheet}


@app.get("/")
def health_check():
    return {"status": "ok", "service": "Excel to Image API", "version": "3.1.0"}
...
async def convert_excel_to_image(
    ...
    # Clean up LibreOffice before each conversion
    cleanup_libreoffice()
    ...
    try:
        ...
        # Convert Excel to PDF using LibreOffice with explicit environment
        logger.info("Starting LibreOffice conversion...")
        
        env = os.environ.copy()
        env['HOME'] = temp_dir  # Use temp dir as HOME to avoid profile issues
        ...
        if not pdf_path.exists():
            ...
            if possible_pdfs:
                ...
                raise HTTPException(
                    status_code=500, 
                    detail=f"LibreOffice conversion failed (code {result.returncode})"
                )
...
async def analyze_excel(file: UploadFile = File(...)):
    ...
    try:
        ...
        return {
            ...
            "message": f"Sheet {analysis['best_sheet']} will be converted"
        }
